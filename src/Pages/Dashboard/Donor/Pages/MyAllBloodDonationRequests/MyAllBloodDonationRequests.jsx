...
const MyAllBloodDonationRequests = () => {
    ...
    useEffect(() => {
      axios.get(`/all-blood-donation-request?email=${user?.email}&page=${currentPageNumber}&limit=${userPerPage}`)
        .then((res) => {
            setMyReqs(res.data);
    ...
    return (
      <div>
        {
            myReqs.length>0?<div><div className="overflow-x-auto">
            <table className="table">
              ...
              <tbody>
                {myReqs.map((req) => (
                  ...
                  <td>
                  {
                        req.donationStatus==='inprogress' && <div className="flex gap-2"><button className="btn btn-error text-white">Cancel</button><button className="btn btn-success text-white">Done</button></div>
                    }
                  </td>
                    <td><Link className="btn btn-info text-white">Edit</Link></td>
                    <td><button className="btn btn-warning text-white">Delete</button></td>
                    <td><Link className="btn btn-primary">View Details</Link></td>
          ...
          </div><div className="py-10  flex justify-center gap-4 ">
